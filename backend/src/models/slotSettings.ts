...
export interface ISlotSettings extends Document {
  ...
  price: number;
  customDayPrice?: ICustomPrice[];
  customDatePrice?: ICustomDatePrice[];
  customDateRangePrice?: ICustomDateRangePrice[];
  isActive: boolean;
  createdAt: Date;
  updatedAt: Date;
}
...
const slotSettingsSchema = new Schema<ISlotSettings>({
  ...
  duration: { type: Number, required: true, min: 5, max: 1440 },
  bookingAllowed: { type: Number, required: true, min: 0, max: 365 },
  price: { type: Number, required: true, min: 0 },
  customDayPrice: { type: [customPriceSchema], default: [] },
...
slotSettingsSchema.index({ isActive: 1 });
...
slotSettingsSchema.pre<ISlotSettings>('save', function(next) {
  ...
  for (const timing of this.timings) {
    ...
    if (startMinutes >= endMinutes) {
      return next(new Error('Start time must be before end time in timings'));
    }
  }
